...
echo "<p class='text-lg'>Price : </p>";
    ...
    <div>
        <form action="process_payment.php" method="post">
            ...
            <button type="submit" name="paymentSubmit" class="inline-flex justify-center w-15 rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                Payment
            </button>
        </form>
    </div>
...
<script>
    document.querySelectorAll('.order-option').forEach(option => {
        option.addEventListener('click', function(e) {
            const mealType = e.target.getAttribute('data-type');
